...
func (tx *Tx) init() {
	tx.CustomFieldDef = NewCustomFieldDefClient(tx.config)
	tx.DimensionOption = NewDimensionOptionClient(tx.config)
	tx.Evidence = NewEvidenceClient(tx.config)
	tx.Item = NewItemClient(tx.config)
	tx.ItemAugment = NewItemAugmentClient(tx.config)
	...
	tx.SyncMeta = NewSyncMetaClient(tx.config)
}
...
// applies a query, for example: CustomFieldDef.QueryXXX(), the query will be executed
